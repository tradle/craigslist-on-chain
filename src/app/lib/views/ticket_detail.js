(function (App) {
	...
	App.View.TicketDetail = Backbone.Marionette.ItemView.extend({
		template: '#ticket-detail-tpl',
		...
		onShow: function () {
			win.info('Show ticket detail (' + this.model.get('pid') + ')');
		...
		toggleFavourite: function (e) {
			...
			var that = this;
			if (this.model.get('bookmarked') === true) {
				Database.deleteBookmark(this.model.get('id'))
					.then(function () {
						win.info('Bookmark deleted (' + that.model.get('id') + ')');
						App.userBookmarks.splice(App.userBookmarks.indexOf(that.model.get('id')), 1);
						that.ui.bookmarkIcon.removeClass('selected').text(i18n.__('Add to bookmarks'));
					})
					.then(function () {
						return Database.deleteTicket(that.model.get('id'));
					})
					.then(function () {
						that.model.set('bookmarked', false);
						var bookmark = $('.bookmark-item .' + that.model.get('id'));
				...
				var ticket = this.model.toJSON();

				Database.addTicket(ticket)
					.then(function () {
						return Database.addBookmark(that.model.get('id'), 'ticket');
					})
					.then(function () {
						win.info('Bookmark added (' + that.model.get('id') + ')');
						that.ui.bookmarkIcon.addClass('selected').text(i18n.__('Remove from bookmarks'));
						App.userBookmarks.push(that.model.get('id'));
